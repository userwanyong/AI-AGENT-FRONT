...
const StyledForm = styled(Form)`
  .semi-form-field {
    margin-bottom: 2px; /* 进一步压缩表单项间距 */
  }
  .semi-input-wrapper {
    height: 38px; /* 略微降低输入框高度 */
  }
  @media (max-width: ${theme.breakpoints.sm}) {
    .semi-input-wrapper {
      height: 34px;
    ...
    try {
      ...
      if (result) {
        ...
        if (typeof result === 'object') {
          ...
          const normalized = {
            id: raw.id ?? raw.userId ?? raw.uid ?? undefined,
            username: raw.username ?? raw.name ?? 'wanyj',
            role: raw.role ?? raw.userRole ?? 2,
          } as any;
          localStorage.setItem('userInfo', JSON.stringify(normalized));
        } else {
          // 如果仅返回布尔值，作为成功登录处理，写入游客信息
          localStorage.setItem('userInfo', JSON.stringify({ username: 'wanyj', role: 2 }));
        }
        localStorage.setItem('isLoggedIn', 'true');
